...
function mergeHeaders(base?: HeadersInit, hasJsonBody: boolean = false) {
...
export async function apiFetch<T>(
  ...
  const headers = mergeHeaders(extraHeaders, hasJsonBody);
  ...
  if (res1.status === 401) {
    ...
    const retryHeaders = mergeHeaders(extraHeaders, hasJsonBody);
